...
trade_matrix <- X_scaled   # this is what we’ll feed into GERGM
...
trade_formula <- trade_matrix_gergm ~
  ...
  absdiff("regulatory_quality_z") + mutual(alpha = 0.05)
...
library(GERGM) # Make sure the library is loaded
...
# --- NEW REQUIRED STEP ---
# Convert the data frame of covariates into a named list of vectors
# This is necessary for the 'covariates' argument in the gergm function
#node_covariates_list <- lapply(as.list(node_covariates), as.vector)
...
```{r}
library(GERGM) # Make sure this is loaded if it's not in the first chunk

gergm_H3b <- gergm(
  formula                        = trade_formula,
  # THIS LINE IS FIXED: Use the correct argument name 'covariate_data'
  covariate_data                 = node_covariates, 
  number_of_networks_to_simulate = 100000,
  thin                           = 1/1000,
  MCMC_burnin                    = 10000, 
  ...
  cores                          = 8,
  control = list(
    auto_tune = TRUE,             # CRITICAL: Enables automatic parameter tuning
    target_accept_rate = 0.25,    # Set target rate
    max_step_size = 0.5,          # Prevents runaway simulations
    auto_tune_steps = 5000        # More steps for tuning to find a stable point
  )
)
